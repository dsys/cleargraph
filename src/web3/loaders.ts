...
import { EthereumNetwork, web3, Web3Address, Web3Block, Web3Transaction } from "./client";
...
export function createWeb3Loaders() {
  return {
    ...
    balance: new DataLoader<Web3HashRequest, number | null>((inputs) => {
      return Promise.all(inputs.map(({ hash, network }) => {
    ...
    blockTransactionCount: new DataLoader<Web3HashRequest, number | null>((inputs) => {
      return Promise.all(inputs.map(async ({ hash, network }) => {
